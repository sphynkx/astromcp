...
local PLANET = {
	sun         = { nom = "Солнце",   gen = "Солнца",   abl = "Солнцем",   retro = "Ретроградное Солнце",  glyph = "\226\152\137" },
	moon        = { nom = "Луна",     gen = "Луны",     abl = "Луной",     retro = "Ретроградная Луна",    glyph = "\226\152\189" },
	mercury     = { nom = "Меркурий", gen = "Меркурия", abl = "Меркурием", retro = "Ретроградный Меркурий", glyph = "\226\152\191" },
	venus       = { nom = "Венера",   gen = "Венеры",   abl = "Венерой",   retro = "Ретроградная Венера",   glyph = "\226\153\128" },
	mars        = { nom = "Марс",     gen = "Марса",    abl = "Марсом",    retro = "Ретроградный Марс",     glyph = "\226\153\130" },
	jupiter     = { nom = "Юпитер",   gen = "Юпитера",  abl = "Юпитером",  retro = "Ретроградный Юпитер",   glyph = "\226\153\131" },
	saturn      = { nom = "Сатурн",   gen = "Сатурна",  abl = "Сатурном",  retro = "Ретроградный Сатурн",   glyph = "\226\153\132" },
	uranus      = { nom = "Уран",     gen = "Урана",    abl = "Ураном",    retro = "Ретроградный Уран",     glyph = "\226\153\133" },
	neptune     = { nom = "Нептун",   gen = "Нептуна",  abl = "Нептуном",  retro = "Ретроградный Нептун",   glyph = "\226\153\134" },
	pluto       = { nom = "Плутон",   gen = "Плутона",  abl = "Плутоном",  retro = "Ретроградный Плутон",   glyph = "\226\153\135" },
	chiron      = { nom = "Хирон",    gen = "Хирона",   abl = "Хироном",   retro = "Ретроградный Хирон",    glyph = "\226\154\183" },
	mean_lilith = { nom = "Лилит",    gen = "Лилит",    abl = "Лилит",     retro = "Ретроградная Лилит",    glyph = "\226\154\184" }, -- несклоняемое
}
...
-- Все четыре теперь латиницей (Asc/Dsc/MC/IC) для единообразия -
-- раньше Асц был кириллицей по буквальному примеру из ТЗ ("Асц в
-- Козероге"), но по факту это разъезжалось с остальными тремя углами.
local ANGLES = {
	{ key = "asc", table_label = "ASC", cat_label = "Asc" },
	...
	if json.fixed_star_conjunctions then
		for _, conj in ipairs(json.fixed_star_conjunctions) do
			...
			if starRu and planetInfo then
				addCat("Звезда " .. starRu .. " в соединении с " .. planetInfo.abl)
			end
		end
	end
...
-- Русские названия фигур Джонса - см. engine/jones_patterns.py на стороне
-- сервиса за полными критериями и источниками по каждой фигуре, включая
-- то, что "Праща" (sling) и "Сгущение" (splay в терминах самого Джонса)
-- - названия, принятые в этом проекте, не общепринятые. "mixed" -
-- НАСТОЯЩИЙ, корректно вычисленный сервисом результат (просто чёткого
-- совпадения ни с одной фигурой нет), а не сбой - в отличие от значения,
-- начинающегося с "Error:" (упала именно эта классификация, а не весь
-- запрос) - оба случая ниже обрабатываются по-разному, см. p.jonesFigure.
local JONES_FIGURE_NAMES_RU = {
	bundle = "Связка",
	sling = "Праща",
...
-- Русское название фигуры Джонса для расположения планет на момент
-- date/time/lat/lon (или city/country) - см. JONES_FIGURE_NAMES_RU выше.
-- Вызов аналогичен p.planetslist/p.aspectslist - те же параметры, тот же
-- принцип "тихой" пустой строки без данных.
--
-- Три разных "пустых" случая различаются намеренно:
--   1) json целиком отсутствует (не хватает параметров, сеть, сервис не
--      ответил) - молча "", как и у остальных функций модуля;
--   2) json есть, но конкретно классификация фигуры на сервисе упала
--      (json.Jones_figure начинается с "Error:") - тоже молча "", а не
--      показ английского текста ошибки на странице; причина остаётся в
--      логах сервиса;
--   3) фигура вычислена, но её английское имя не найдено в
--      JONES_FIGURE_NAMES_RU (например, сервис добавил новую фигуру, а
--      этот словарь ещё не обновили) - НЕ прячем: показываем как есть,
--      английским именем, чтобы факт появления новой фигуры был виден
--      на странице, а не потерялся молча.
function p.jonesFigure(frame)
	local args = resolveArgs(frame)
	local json = p._getData(args)
	if not json or isBlank(json.Jones_figure) then
		return ""
	end

	local figure = json.Jones_figure
	if figure:sub(1, 6) == "Error:" then
		return ""
	end

	-- Русское имя резолвится в отдельную переменную ДО сборки строки:
	-- "A .. B or C" в Lua - это НЕ "(A .. B) or C" с полноценным откатом,
	-- как можно было бы подумать по аналогии с "and/or" - конкатенация
	-- (..) на nil-значении не возвращает nil тихо, а падает с ошибкой
	-- целиком. Раньше (простой вариант, без таблицы/иконки) это не
	-- проявлялось, потому что "or figure" стоял последним и отрабатывал
	-- как задумано. Сейчас, при сборке многосоставной HTML+категория
	-- строки, "or" внутри такой цепочки .. уже не спасает - нужно
	-- сначала разрешить имя (с откатом на английский слаг), а потом уже
	-- использовать готовую строку везде, где нужно название фигуры.
	local nameRu = JONES_FIGURE_NAMES_RU[figure] or figure

	return '<table border=0 width="160px" style="text-align: center; margin: 10px auto;"><tr><td>' .. nameRu ..
		"</td><td>[[File:J" .. figure .. ".png|32px]]</td></tr></table>[[Category:Фигура Джонса: " .. nameRu .. "]]"
end
