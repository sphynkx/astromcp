...
def run_scan(
    ...
    fixed_offset = resolve_fixed_offset_minutes(
        ...
        natal_year, natal_month, natal_day, scan_start_hour, scan_start_minute, 0,
    )

    start_total_min = scan_start_hour * 60 + scan_start_minute
    end_total_min = scan_end_hour * 60 + scan_end_minute
    if end_total_min < start_total_min:
        end_total_min += 24 * 60  # allow wrap past midnight

    candidates = list(range(start_total_min, end_total_min + 1, step_minutes))

    results = []
    for total_min in candidates:
        cand_hour = (total_min // 60) % 24
        cand_minute = total_min % 60

        natal_subject, _, _ = build_subject(
            "natal", natal_year, natal_month, natal_day, cand_hour, cand_minute, 0,
        ...
        for ev in events:
            ...
            if technique == "transit":
                ...
                computed, natal_pts, meta = technique_secondary_progression(
                    natal_year, natal_month, natal_day, cand_hour, cand_minute, 0,
